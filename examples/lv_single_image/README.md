...
- Week 21: Memory Second Prototype — consolidate raw moments into durable memory files via DeepSeek.
- Week 22: Memory-aware conversation — bot references shared memories, asks follow-ups, and knows when to end.
...
## Week 22: Memory-Aware Conversation

The bot now reads its consolidated memory files before each reply, making conversations more authentic:

- **Memory recall**: references shared history ("你上次说过...", "我记得你喜欢...")
- **Follow-up questions**: asks return questions to keep conversation flowing
- **Natural endings**: detects when the conversation is complete and signs off gracefully
- **Conversation timeout**: logs when the user doesn't reply within a set time

### Usage

Memory injection is automatic when `--bot-url` is set. The bot fetches `companion_self.json`, `user_facts.json`, `shared_phrases.jsonl`, `boundaries.json`, and `backstory_fragments.json` from the device.

```sh
# The bot will remember your past interactions
python3 tools/ask_bot.py --bot-url 192.168.1.138 "你还记得自己叫什么名字吗"

# Disable memory injection
python3 tools/ask_bot.py --bot-url 192.168.1.138 --no-memory "hello"

# Wait for user reply; log timeout if none given
python3 tools/ask_bot.py --bot-url 192.168.1.138 --conversation-timeout 30 "今天天气真不错"
```

### Conversation End Detection

When the bot sets `conversation_end: true`, a `conversation_end` event is automatically logged to `moments.jsonl`. If `--conversation-timeout` is set and the user doesn't reply (Ctrl+C), a `conversation_timeout` event is logged instead.

```sh
python3 tools/ask_bot.py --bot-url 192.168.1.138 "好的晚安"
# → Bot state: SLEEPY (conversation ending)
# → No follow-up, warm sign-off
```
...
For smoother playback, prefer native 480px fast-path GIFs generated by `tools/generate_eye_assets.py`. Avoid software-zoomed 128px loops and large full-color GIFs; both can feel laggy on the ESP32-S3.
...
The current T-RGB-friendly target is a native 480x480 fast GIF loop: about 14 frames, 28 fps, 16 colors, and usually under 80 KB per state.

```sh
python3 tools/generate_eye_assets.py --width 480 --height 480 --eye-scale 0.85 --fps 28 --seconds 0.50
```
...
```sh
python3 tools/generate_eye_assets.py --width 480 --height 480 --eye-scale 0.85 --fps 28 --seconds 0.50
python3 tools/upload_asset.py --bot-url 192.168.1.123 --folder /data/gif_loops/480 examples/lv_single_image/data/eye_assets_perf_fast/480x480/*_loop.gif --no-resize --list
python3 tools/bind_asset.py --bot-url 192.168.1.123 --folder /data/gif_loops/480 IDLE idle_loop.gif
```

Keep expressions eye-led and cohesive: blink, eye shape, gaze, pose, brightness pulse, and a small number of integrated props. Avoid thin mouth marks, floating punctuation signs, full-screen video, large GIFs, and high-color gradients when testing on ESP32.

The generated states intentionally use distinct readable cues:

- `attention`: wider wake-up eyes and a small alert pulse.
- `listening`: a side receiver dish with small wave pulses.
- `ack`: focused eyes with an optional OK-hand cue.
- `thinking`: raised brow, thought bubble, tiny antenna, and soft particles.
- `deep_think`: slower pupils and orbiting thought dots.
- `recall`: dimmer memory lookup with an archive cue.
- `speaking`: pulsing eyes and side speech waves.
- `teasing`: clean wink and raised brow.
- `annoyed`: compressed eyes, heavy brows, fume tick, and tiny shake.
- `proud`: cyan star eyes.
- `delight`: cyan heart eyes and sparkle.
- `concern`: inner-brow feel and softened eyes.
- `sleepy`: crescent lids with a floating `Z`.
- `memory`: closed eyes, antenna pulse, memory ring, and particles.
- `uncertain`: asymmetric eyes and small sweat drop.
- `boundary`: flattened eyes and a small stop-hand cue.
- `misheard`: receiver, sweat drop, and retry/listen wobble.
- `initiate`: bright eyes with a small wave cue.
- `camera_curious`: pupils inside a scan frame.
